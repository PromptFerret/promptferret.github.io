...
function renderDetails(rowData, highlightText = "") {
    ...
    let html = `
        ...
        <div class="d-none"><b>Rarity:</b> ${copy(rarity)}</div>
        <div><b>Cost:</b> ${copy(cost)}</div>
        <div class="d-none"><b>Requires Attunement:</b> ${copy(atnVal ? "Yes" : "No")}</div>
        <div class="d-none"><b>Session Required:</b> ${copy(sessVal ? "Yes" : "No")}</div>
    `;
...
function setupEvents() {
    ...
    document.getElementById("order-btn").addEventListener("click", () => {
        ...
        // let output = `${characterName} buys:\n${lines.map((l) => `     （${l.qty}x） ${l.displayName} ${l.perItem.toLocaleString()} – ${l.itemTotal.toLocaleString()} PP`).join("\n")}\nTotal ${total.toLocaleString()} PP`;
        let output = `${characterName} buys:\n${lines.map((l) => `     ${l.displayName} ${l.perItem.toLocaleString()} – ${l.itemTotal.toLocaleString()} PP`).join("\n")}\nTotal ${total.toLocaleString()} PP`;
...
function renderCart() {
    ...
    if (cart.length === 0) {
        ...
        if (totalEl) totalEl.textContent = "Total: 0 PP";
        return;
    }
        ...
        html += `<tr>
            ...
            <td>
                <input type="text" inputmode="numeric" pattern="[0-9]*"
                    class="form-control form-control-sm cart-qty d-none"
                    style="max-width: 60px; min-width: 40px; display:inline-block;"
                    data-idx="${originalIdx}" value="${item.quantity ?? ""}">1
            </td>
            <td>
                ${
                    showBase
    ...
    if (totalEl) totalEl.textContent = `Total: ${grandTotal.toLocaleString()} PP`;
